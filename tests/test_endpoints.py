...
def test_create_parking(client):
    ...
    new_parking = {"data": {"address": "101 Main st", "count_spaces": "20"}}
    headers = {"contentType": "application/json"}
...
def test_entering_parking(client, db):
    ...
    free_spaces_before = (
        db.session.query(Parking).filter_by(id="1").first().count_available_places
    )
    new_log = {"data": {"client_id": "2", "parking_id": "1"}}
    headers = {"contentType": "application/json"}
...
def test_leaving_parking(client_id, status_code, client, db):
    ...
    new_log = {"data": {"client_id": client_id, "parking_id": "1"}}
    headers = {"contentType": "application/json"}
    ...
    assert (
        db.session.query(Client).filter_by(id=client_id).first().credit_card is not None
    )
